...
pub fn serialize<W: Write, S: serde::Serialize>(out: W, val: S) -> Result<(), Error> {
	val.serialize(&mut Serializer::new(out))
}

fn check_valid_name(_name: &str) -> Result<(), Error> { Ok(()) }
fn check_valid_attr(_name: &str) -> Result<(), Error> { Ok(()) }
...
impl<'a, W: Write> serde::ser::SerializeStruct for Struct<'a, W> {
	...
	fn serialize_field<T: ?Sized + serde::Serialize>
		(&mut self, key: &'static str, value: &T) -> Result<(), Self::Error>
	{
...
impl<'a, 'b, W: Write> SubSerializer<'a, 'b, W> {
	fn attr<F: FnOnce(&mut Serializer<W>)->Result<(), Error>>(&mut self, f: F) -> Result<(), Error> {
	...
	fn child<F: FnOnce(&mut Serializer<W>)->Result<(), Error>>(&mut self, f: F) -> Result<(), Error> {
		check_valid_name(self.name)?;
...
impl<'a, 'b, W: Write> serde::ser::Serializer for SubSerializer<'a, 'b, W> {
	...
	fn serialize_some<T: ?Sized + serde::Serialize>(mut self, value: &T) -> Result<Self::Ok, Self::Error> {
